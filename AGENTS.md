...
**Completed This Session:** Removed the hardcoded admin email from `/admin/login`, reduced admin login autofill by disabling autocomplete on the form fields, added stricter shared calendar-date validation, normalized admin login email input, tightened booking time/date validation, added client-side max lengths for booking text inputs, and reused strict date validation for availability and blocked-date APIs. Verified `npm run lint`, `npm run build`, invalid availability date `400`, invalid booking date `400`, invalid admin login `400`, valid Saturday availability `200`, and no Sanchit email string rendered in the admin login HTML.
