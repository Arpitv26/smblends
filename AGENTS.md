...
**Completed This Session:** Added a shared strict `HH:MM:SS` time-slot validator, reused it in booking validation and slot generation, rejected past booking dates with a friendly `400`, and filtered public availability so past dates return no slots and same-day slots that have already started no longer appear. Then updated free-launch email behavior: the booking form keeps optional email collection but no longer promises client confirmation emails, and Resend now sends only the barber notification until a verified sending domain exists. Disabled the Design add-on at the shared booking config level so it disappears from the UI and is rejected by the API, while leaving the database schema flexible for re-enabling later. Verified `npm run lint`, `npm run build`, invalid booking time `400`, past booking date `400`, past availability date returns an empty slot list, and direct Design add-on submission returns `400`. User also reported applying the Supabase partial unique index query for cancelled-slot rebooking and confirmed Sanchit receives barber notification emails with Sanchit's Resend API key.
