...
**Completed This Session:** Added a shared strict `HH:MM:SS` time-slot validator, reused it in booking validation and slot generation, rejected past booking dates with a friendly `400`, and filtered public availability so past dates return no slots and same-day slots that have already started no longer appear. Then updated free-launch email behavior: the booking form keeps optional email collection but no longer promises client confirmation emails, and Resend now sends only the barber notification until a verified sending domain exists. Disabled the Design add-on at the shared booking config level so it disappears from the UI and is rejected by the API, while leaving the database schema flexible for re-enabling later. Fixed mobile iPhone date input overflow in the booking preview by wrapping and clipping the native date input. Added Cloudflare Workers/OpenNext deployment setup with pinned `@opennextjs/cloudflare@1.15.0`, `wrangler@4.59.2`, `wrangler.jsonc`, `open-next.config.ts`, static asset cache headers, and deploy scripts. Deployed to Cloudflare Workers at `https://booking.smblends.workers.dev`. Fixed production admin timezone display bugs where `21:00:00` rendered as `2:00 p.m.` by formatting admin booking and availability time strings directly as wall-clock times instead of converting through `Date`. Verified `npm run lint`, `npm run build`, `npx opennextjs-cloudflare build`, live homepage `200`, live availability API `200`, invalid booking time `400`, past booking date `400`, past availability date returns an empty slot list, and direct Design add-on submission returns `400`. User also reported applying the Supabase partial unique index query for cancelled-slot rebooking and confirmed Sanchit receives barber notification emails with Sanchit's Resend API key.
